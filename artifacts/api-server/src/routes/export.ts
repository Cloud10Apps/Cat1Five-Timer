...
router.get("/elevators", asyncHandler(async (req, res) => {
  ...
  const rows = await db
    .select({
      ...
      unitElevatorType: elevatorsTable.elevatorType,
  ...
  sheet.columns = [
    ...
    { header: "Elevator Type",      key: "unitElevatorType",   width: 16 },
    { header: "OEM Serial No.",     key: "oemSerialNumber",    width: 20 },
    { header: "Capacity",           key: "capacity",           width: 14 },
    { header: "Speed",              key: "speed",              width: 14 },
    { header: "Year Installed",     key: "yearInstalled",      width: 16 },
    { header: "# Landings",         key: "numLandings",        width: 12 },
    { header: "# Openings",         key: "numOpenings",        width: 12 },
  ...
  rows.forEach(r => {
    ...
    sheet.addRow({
      ...
      unitElevatorType:  r.unitElevatorType  ?? "",
      oemSerialNumber:   r.oemSerialNumber   ?? "",
      capacity:          r.capacity          ?? "",
      speed:             r.speed             ?? "",
      yearInstalled:     r.yearInstalled      ?? "",
      numLandings:       r.numLandings       ?? "",
      numOpenings:       r.numOpenings       ?? "",
